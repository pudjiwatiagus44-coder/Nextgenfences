...
class FenceWidget(QWidget):
    ...
    def __init__(self, fence_id, title="新分区", parent=None, opacity=0.1, view_mode="icon_medium", custom_order=None, font_size=12, sort_by="name", sort_order="asc"):
        ...
        if parent:
            ...
            # 浣跨敤 FramelessWindowHint
            # 涓嶄娇鐢?StaysOnBottomHint锛岄伩鍏嶈妗岄潰鍥炬爣瑕嗙洊
            self.setWindowFlags(
                Qt.WindowType.FramelessWindowHint | Qt.WindowType.Tool
            )
            
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
        self.setAttribute(Qt.WidgetAttribute.WA_ShowWithoutActivating, True)  # 涓嶆縺娲荤獥鍙?        
        ...
        if not parent:
            # 鏆傛椂绂佺敤妗岄潰宓屽叆锛岄伩鍏岶ence绐楀彛娑堝け鐨勯棶棰?            # QTimer.singleShot(500, self.attach_to_desktop)
            ...
            # 寤惰繜 500ms 纭繚绐楀彛宸插垱寤?            # QTimer.singleShot(500, self.attach_to_desktop)
            
            # 娣诲姞瀹氭椂鍣ㄧ‘淇濈獥鍙ｅ缁堝彲瑙?            # self.visibility_timer = QTimer(self)
            # self.visibility_timer.timeout.connect(self.ensure_visible)
            # self.visibility_timer.start(5000)  # 姣?5 绉掓鏌ヤ竴娆?
    def check_attachment(self):
        """瀹氭湡妫€鏌ュ祵鍏ョ姸鎬侊紝纭繚绐楀彛浠嶇劧闄勭潃鍦ㄦ闈㈠眰"""
        try:
            hwnd = int(self.winId())
            if not win32gui.IsWindow(hwnd):
                # 绐楀彛宸茶閿€姣侊紝鍋滄瀹氭椂鍣?                if hasattr(self, 'attachment_timer'):
    ...
    def attach_to_desktop(self):
        """灏?分区窗口宓屽叆鍒?Windows 妗岄潰灞?""
        try:
            hwnd = int(self.winId())
            
            # 璋冪敤 ZOrderManager 杩涜宓屽叆
            ...
            if success:
                ...
                # 澶囩敤鏂规锛氬皢绐楀彛鎻愬崌鍒伴《灞?                win32gui.SetWindowPos(
                    hwnd,
    ...
    def ensure_visible(self):
        ...
        try:
            ...
            if win32gui.IsWindow(hwnd):
                # 濡傛灉绐楀彛涓嶅彲瑙侊紝鏄剧ず瀹?                if not win32gui.IsWindowVisible(hwnd):
    ...
    def set_path(self, path):
        ...
        if self.current_path and os.path.exists(self.current_path):
            ...
            self.refresh_timer.start(5000)  # 姣?5 绉掑埛鏂颁竴娆?            self.refresh_files()
    ...
    def dropEvent(self, event):
        ...
        if mime.text().startswith("internal_move:"):
            ...
            if file_path in self.custom_order:
                ...
                # 璁剧疆涓鸿嚜瀹氫箟鎺掑簭妯″紡锛岄槻姝㈣嚜鍔ㄥ埛鏂版椂閲嶆柊鎺掑簭
                if self.sort_by != "custom":
                    self.sort_by = "custom"
                    # 淇濆瓨鎺掑簭鏂瑰紡鍒伴厤缃紝纭繚閲嶅惎鍚庝繚鎸佽嚜瀹氫箟鎺掑簭
                    self.sort_changed.emit(self.fence_id, self.sort_by, self.sort_order)
